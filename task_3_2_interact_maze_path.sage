...
@interact
def build_path_interact(seed = input_box('14', type = int, label = 'seed (int)'), \
                        width = input_box('8', type = int, label = 'width (int)'), \
                        height = input_box('12', type = int, label = 'height (int)')):
    random.seed(seed)
    maze = ''
    for i in range(height):
        ...
        for j in range(width):
            if (i > 0 or j > 0) and (i < height - 1 or j < width - 1):
                ...
                line += '.'
        maze += line + "\n"
    build_path(maze, [0, 0], [width - 1, height - 1])
